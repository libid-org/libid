...
Verifier Dispatcher: The component on the Execution Ledger that every
   ...
   identity platform and LOPPV Version; it selects the LOPPV registered for
...
Ledger OAuth Platform Proof Verifier (LOPPV): The component on the Execution
   Ledger registered for one identity platform and one LOPPV Version. It
   validates that version's OAuth Proof and returns its verified identity
   outputs. It obtains attestation authenticity from the Notary Service for
   each attestation its Platform Profile requires, which is no attestation at
   all where that profile carries none.

LOPPV Version: The unsigned 16-bit `platformVerifierVersion`
   selecting one identity platform's proof statement and LOPPV. It is not a
   version of the ceremony process or a mutable verifier-authority revision.

Supported Version Set: The identity-platform and LOPPV Version
...
OAuth Proof: The complete input an Identity Integration passes to the Verifier
   Dispatcher for one verification: the identity platform, the LOPPV Version,
...
Platform Profile: The immutable, independently versioned definition of one
   ...
   of those two. Its constants are fixed in the LOPPV registered
   for that platform and version, not in the Identity Integration.
...
Notary Service: The role that observes a TLS session and signs the resulting
   ...
   proof-specific and belong to the LOPPV. A Platform Profile
...
the selected LOPPV, including this Authorization Digest layout.

- REQ-COMMON-01B (upholds SP-BIND-01):
  The Verifier Dispatcher MUST reject an OAuth Proof whose identity platform and
  `platformVerifierVersion` pair lies outside its Supported Version Set. An
  OAuth Proof change bumps that platform's version; a common Authorization
...
- REQ-COMMON-02 (upholds SP-BIND-01):
  ...
  caller-supplied operation domain and LOPPV Version, its observed
...
field checking. The Supported Version Set lives in the Verifier Dispatcher, and
every platform constant — endpoints, revealed ranges, trust roots, parameters
— lives in the LOPPV registered for that platform and version.
The Identity Integration holds none of them.
...
- REQ-COMMON-05:
  ...
  LOPPV Version, the OAuth Proof, and the native value the
...
- REQ-COMMON-05A:
  ...
  verifies nothing.
- REQ-COMMON-05B:
  The Verifier Dispatcher MUST support more than one LOPPV Version of
...
- REQ-COMMON-45 (upholds SP-BIND-01, SP-EXCHANGE-01):
  ...
  the submitted identity platform and LOPPV Version. The LOPPV MUST reject an
...
- REQ-COMMON-06C (upholds SP-BIND-01):
  ...
  MUST dispatch on the LOPPV Version the OAuth Proof names.
...
- REQ-COMMON-06E:
  ...
  LOPPV Version covering the whole verification path, quoting one
...
- REQ-COMMON-12 (upholds SP-BIND-01):
  ...
  this construction is a change to the proof statement, which bumps
  `platformVerifierVersion` and its LOPPV.
...
- REQ-COMMON-19E (upholds SP-BIND-01):
  ...
  proof, attestation, identity platform, LOPPV Version, or other
...
- TEST-COMMON-22 (exercises REQ-COMMON-45):
  ...
  its identity platform and LOPPV Version is rejected; a proof
...
digest; a profile collision forfeits that separation. Replay across LOPPV Versions is prevented by
