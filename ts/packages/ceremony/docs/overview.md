...
**Release qualification is incomplete.** Real proof generation and
static-distribution checks are separate from real OAuth, matched-notary concurrency
and device qualification. See [Qualification](qualification.md) for evidence,
...
synthetic fixture; the package does not claim those fixtures are production ledger definitions.
...
See the [architecture module map](architecture.md#ownership) and
[source guide](source.md). Prefetch imports only data-only asset declarations;
execution imports those same handles. The closed compiler graph supplies the
actual runtime chunks and nested-worker edges.
...
Owner-defined archive URLs pin circuit v0.3.0 and notary v0.3.0-rc.3 releases.
...
headers and Brotli/gzip negotiation. Configure the independently deployed OAuth Bridge
according to [OAuth Bridge](oauth-bridge.md); ceremony supplies no production
