...
## Popup and callback documents

`GET /api/v1/ceremony/popup` and the configured callback path directly serve
the same deployment-generated bytes and security headers. The callback path is
an alias, not an HTTP redirect. The OAuth registration uses the callback URL,
while application launch uses `/api/v1/ceremony/popup`.
...
The popup and callback response uses:
