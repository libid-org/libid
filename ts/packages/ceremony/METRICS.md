...
Callback, Airlock, and Prover send only bounded, sanitized diagnostic records
...
package must not pass an OTLP endpoint into Callback, Airlock, or Prover and
...
| proof handoff | authenticated OAuth delivery through Airlock navigation and `ProverReady` |
...
| post-callback join | Airlock entry through Prover selected-flight readiness |
...
- popup-connection continuity through Callback-to-Airlock-to-Prover navigation;
