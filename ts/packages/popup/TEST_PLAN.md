...
| POPUP-CONTROL-002 | After direct control is severed, application-endpoint `navigate` sends one exact canonical-HTTPS `Navigate`; popup-endpoint `navigate` sends no control. Either popup-side path invokes replacement only after carrier continuity is preserved or prepared; malformed, credentialed, noncanonical, relative, non-HTTPS, and unpreparable navigations fail before browser navigation. |
