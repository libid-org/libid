...
| POPUP-KEEPER-005 | `PopupWindow.current(fragment, { scope })` uses only the registration with exactly that same-origin scope for claim, keep, and activation waits, never the registration controlling the document; with a root and a nested registration sharing one script URL, continuity goes through the named root. A scope absent at construction is found once registered, a root still installing is waited for up to the keeper reply deadline, an absent root yields no worker even when the nested one is active, and a cross-origin scope rejects synchronously. |
