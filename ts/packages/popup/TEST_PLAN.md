...
| POPUP-CONNECTION-014 | With the opener severed and a fallback carrier selected, a document requiring isolation prepares its carrier's successor, retires it, and replaces itself with the fallback; the isolated destination authenticates a fresh carrier through its own constructor, the application installs the reported successor without ending the logical connection or reusing the retired carrier, and both directions deliver. The intermediate document never becomes ready or delivers. Close during preparation navigates nothing; a failed reconnection rejects only the destination's `ready`, and values the application sent meanwhile are lost, not replayed; a fallback document that stays non-isolated fails with `isolation-unavailable`. |
